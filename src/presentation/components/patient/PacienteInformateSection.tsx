...
export function PacienteInformateSection() {
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {INFO_CARDS.map((card, index) => (
          <InfoCard 
            ...
            {...card} 
            onButtonClick={
              index === 0 ? () => router.push('/paciente/ansiedad') :
              index === 1 ? () => router.push('/paciente/salud-mental') :
              index === 2 ? () => router.push('/paciente/test') :
              undefined
            }
          />
        ))}
